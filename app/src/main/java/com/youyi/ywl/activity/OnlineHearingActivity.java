...
//                    PostList();//在请求筛选数据成功后 再请求列表数据接口
    ...
    private void initListRecyclerView() {
        ...
        onlineHearingAdapter.setOnItemClickListener(new RecyclerViewOnItemClickListener() {
            ...
            public void OnItemClick(View view, int position) {
                //点击条目记录到学习轨迹数据库表格中
                LearningPathDao2 learningPathDao2 = new LearningPathDao2(OnlineHearingActivity.this, TABLE_NAME, DB_NAME);
                learningPathDao2.insert(dataList.get(position));
    ...
    @OnClick({R.id.ll_back, R.id.iv_right, R.id.ll_shaixuan_second})
