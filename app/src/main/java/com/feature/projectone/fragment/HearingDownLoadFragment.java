...
public class HearingDownLoadFragment extends BaseFragment {
    ...
    public HearingDownLoadFragment() {
    }

    @SuppressLint("ValidFragment")
    public HearingDownLoadFragment(String cate_id) {
    ...
    private void initRecyclerView() {
        ...
        sourceDownloadFragmentAdapter.setOnDownloadIconListener(new DownloadIconClickListener() {
            ...
            public void OnDownLoadClick(SourceDownloadFragmentAdapter.MyViewHolder holder, View view, int position) {
                ...
                HearingDownLoadFragment.this.holder = holder;
                if (ShareUtil.isExist(getActivity(), Constanst.UER_TOKEN)) {
                    if (PermissionUtil.checkPermission(getContext(), HearingDownLoadFragment.this, Manifest.permission.WRITE_EXTERNAL_STORAGE, writePermissionCode)) {
                        //有权限直接操作
                        PostDownloadList(id);
                    } else {
        ...
        switch (requestCode) {
            case writePermissionCode:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (!HearingDownLoadFragment.this.shouldShowRequestPermissionRationale(permissions[0])) {
