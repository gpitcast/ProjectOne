...
public class WordDictationFragment extends BaseFragment {
    ...
    public WordDictationFragment() {
    }

    @SuppressLint("ValidFragment")
    public WordDictationFragment(String cate_id) {
    ...
    private void initRecyclerView() {
        ...
        sourceDownloadFragmentAdapter.setOnDownloadIconListener(new DownloadIconClickListener() {
            ...
            public void OnDownLoadClick(SourceDownloadFragmentAdapter.MyViewHolder holder, View view, int position) {
                ...
                WordDictationFragment.this.holder = holder;
                if (ShareUtil.isExist(getActivity(), Constanst.UER_TOKEN)) {
                    if (PermissionUtil.checkPermission(getContext(), WordDictationFragment.this, Manifest.permission.WRITE_EXTERNAL_STORAGE, writePermissionCode)) {
                        //有权限直接操作
                        PostDownloadList(id);
                    } else {
        ...
        switch (requestCode) {
            case writePermissionCode:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (!WordDictationFragment.this.shouldShowRequestPermissionRationale(permissions[0])) {
